 ...
 * @author mabrams
 */
public class CougaarPreferencePage extends PreferencePage implements
		IWorkbenchPreferencePage, IAddCougaarDialogRequestor,
		ISelectionProvider {
	...
	public CougaarPreferencePage() {
		...
		setTitle(CougaarPreferencesMessages
				.getString("CougaarPreferencePage.Installed_CougaarVersions_1"));
	...
	protected Control createContents(Composite ancestor) {
		...
		tableLabel.setText(CougaarPreferencesMessages
				.getString("CougaarPreferencePage.Installed_&Cougaars__1")); //$NON-NLS-1$
		data = new GridData();
		...
		final String[] variableTableColumnHeaders = {
				CougaarPreferencesMessages
						.getString("cougaarPreferencePage.cougaarName"),
				CougaarPreferencesMessages
						.getString("cougaarPreferencePage.cougaarLocation") };

		final ColumnLayoutData[] variableTableColumnLayouts = {
				new ColumnWeightData(25), new ColumnWeightData(75) };
		...
		fAddButton.setText(CougaarPreferencesMessages
				.getString("cougaarPreferencePage.add")); //$NON-NLS-1$
		...
		fEditButton.setText(CougaarPreferencesMessages
				.getString("cougaarPreferencePage.edit")); //$NON-NLS-1$
		...
		fRemoveButton.setText(CougaarPreferencesMessages
				.getString("cougaarPreferencePage.remove")); //$NON-NLS-1$
	...
	private void addCougaar() {
		...
		dialog.setTitle(CougaarPreferencesMessages
				.getString("cougaarPreferencePage.addCougaar.title")); //$NON-NLS-1$
	...
	private void enableButtons() {
		...
		int selectionCount = ((IStructuredSelection) fCougaarList
				.getSelection()).size();
	...
	private void editCougaar() {
		...
		AddCougaarDialog dialog = new AddCougaarDialog(this, getShell(),
				cougaarInstall);
		dialog.setTitle(CougaarPreferencesMessages
				.getString("CougaarPreferencePage.editVersion.title"));
		if (dialog.open() != Window.OK) {
			return;
		}
	 ...
	 * 
	 * @param cougaar
	 *            the cougaar installation that was removed
	 */
